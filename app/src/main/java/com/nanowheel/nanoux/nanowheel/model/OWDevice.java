...
public class OWDevice extends BaseObservable implements DeviceInterface {
    ...
    public static final String KEY_ODOMETER_CHARGE = "odometer_charge";
    public static final String KEY_ODOMETER_RANGE = "odometer_range";
...
*/
    ...
    public static class DeviceCharacteristic {
        ...
        public DeviceCharacteristic(String uuid, String key, String ui_name, int state) {
            this.uuid.set(uuid);
            this.key.set(key);
    ...
    public void setupCharacteristics(Context context) {
        ...
        /*deviceReadCharacteristics.add(new DeviceCharacteristic(OnewheelCharacteristicLifetimeOdometer, KEY_LIFETIME_ODOMETER,   "LIFETIME ODOMETER"));            // 2
        ...
        deviceReadCharacteristics.add(new DeviceCharacteristic(OnewheelCharacteristicCustomName,     CUSTOM_NAME,            ""));                                //8
        ...
        deviceNotifyCharacteristics.add(new DeviceCharacteristic(MockOnewheelCharacteristicOdometerCharge,     KEY_ODOMETER_CHARGE,            "", false)); //18
        deviceNotifyCharacteristics.add(new DeviceCharacteristic(MockOnewheelCharacteristicOdometerRange,     KEY_ODOMETER_RANGE,            "", false)); //19
        deviceNotifyCharacteristics.add(new DeviceCharacteristic(OnewheelCharacteristicSafetyHeadroom,     KEY_SAFETY_HEADROOM,            "", false)); //20
        deviceNotifyCharacteristics.add(new DeviceCharacteristic(MockOnewheelCharacteristicCharging,     KEY_CHARGING,            "", false)); //21
        deviceNotifyCharacteristics.add(new DeviceCharacteristic(OnewheelCharacteristicUartSerialRead,   KEY_SERIAL_READ,         ""));                         //22,18 in ponewheel src
*/
        ...
        deviceReadCharacteristics.add(new DeviceCharacteristic(OnewheelCharacteristicBatteryVoltage, KEY_BATTERY_VOLTAGE,   "Battery Voltage",4));
    ...
    public void refreshCharacteristics(Context context) {
        boolean isMetric = App.INSTANCE.getSharedPreferences(context).isMetric();
    ...
    final double KmToMi = 0.62137119;
...
//            }
//        }
        NotificationBuilder.sensorWarning(BluetoothService.mContext);
    ...
    public void processOdometer(byte[] incomingValue, DeviceCharacteristic dc) {
        ...
        if (i_odometer > App.INSTANCE.getSharedPreferences().getDistanceRecord()){
            App.INSTANCE.getSharedPreferences().setDistanceRecord(i_odometer);
        }
    }
    ...
    public void processSpeedRpm(byte[] incomingValue, DeviceCharacteristic dc) {
        ...
        //App.INSTANCE.getSharedPreferences().setSpeedRecord(0);
        NotificationBuilder.speedNotification(BluetoothService.mContext);
        NotificationBuilder.speedWarning(BluetoothService.mContext);
        NotificationBuilder.speedWarning2(BluetoothService.mContext);
        if (i_speedRpm > App.INSTANCE.getSharedPreferences().getSpeedRecord()){
            App.INSTANCE.getSharedPreferences().setSpeedRecord(i_speedRpm);
        }
    }
...
//        Timber.d("controllerTemp = " + controllerTemp);
    ...
    private void processLifetimeOdometer(byte[] incomingValue, DeviceCharacteristic dc) {
        ...
        App.INSTANCE.getSharedPreferences().setDistanceTotal(i_lifetime);
        if (App.INSTANCE.getSharedPreferences().isMetric()) {
    ...
    float lastDist = -1;
    private void updateChargeDist( float revs){
        //App.INSTANCE.getSharedPreferences().setChargeDistRev(0);
        if (lastDist < 0){
            if (characteristics.get(OnewheelCharacteristicBatteryRemaining).value.get() != null){
                ...
                float dist = revs;
                if (App.INSTANCE.getSharedPreferences().getChargeDistBat() >= battery){
                    lastDist = dist;
                    App.INSTANCE.getSharedPreferences().setChargeDistBat(battery);
                    //App.INSTANCE.getSharedPreferences().setChargeDistRev(App.INSTANCE.getSharedPreferences().getChargeDistRev()+dist);
                }else{
                    lastDist = dist;
                    App.INSTANCE.getSharedPreferences().setChargeDistRev(dist);
                    App.INSTANCE.getSharedPreferences().setChargeDistBat(battery);
                }
                if (App.INSTANCE.getSharedPreferences().isMetric()) {
                    characteristics.get(MockOnewheelCharacteristicOdometerCharge).value.set(Util.revolutionsToKilometers(App.INSTANCE.getSharedPreferences().getChargeDistRev()) + "");
                }else{
                    characteristics.get(MockOnewheelCharacteristicOdometerCharge).value.set(Util.revolutionsToMiles(App.INSTANCE.getSharedPreferences().getChargeDistRev()) + "");
                }
            }
        }else{
            float battery = Util.parseF(characteristics.get(OnewheelCharacteristicBatteryRemaining).value.get());
            ...
            App.INSTANCE.getSharedPreferences().setChargeDistBat(battery);
            App.INSTANCE.getSharedPreferences().setChargeDistRev(App.INSTANCE.getSharedPreferences().getChargeDistRev()+(dist-lastDist));
            lastDist = dist;
    ...
    public void updateChargeDistCharging(){
        App.INSTANCE.getSharedPreferences().setChargeDistRev(0);
    }
    ...
    private void calcRange(int battery){
        ...
        if (characteristics.get(MockOnewheelCharacteristicCharging).value.get() == "true"){
            battStart = -1;
            battStartCalc = -1;
            rStart = -1;
    ...
    static String[] modesList = {"Uh Oh","Classic","Extreme","Elevated","Sequoia","Cruz","Mission","Elevated","Delirium","Custom"};
    static String[] modeSpeeds = {"Uh oh","12","15","15","12","15","19","19","20",""};
    public static void speedChange(View view,int ID, int ID2){
        final TextView speedText = view.findViewById(ID);
        String value = BluetoothService.mOWDevice.characteristics.get(MockOnewheelCharacteristicSpeed).value.get();
    ...
    public static void modeChange(View view, int ID, int ID2){
        final TextView textMode = view.findViewById(ID);
        String value = BluetoothService.mOWDevice.characteristics.get(OnewheelCharacteristicRidingMode).value.get();
        if(value == null)  return;
        final int disVal = Math.round(Util.parseF(value));
    ...
    public static void batteryChange(Context context){

        String value = BluetoothService.mOWDevice.characteristics.get(OnewheelCharacteristicBatteryRemaining).value.get();
        if(value == null)  return;
        final int percent = Util.parseI(value);
        BluetoothService.updateNotification(percent,context);
    }
    public static void batteryChange(View view,int ID, int ID2){
    ...
    public static void maxSpeedChange(View view,int ID){
        final TextView test = view.findViewById(ID);
        String value = BluetoothService.mOWDevice.characteristics.get(MockOnewheelCharacteristicMaxSpeed).value.get();
        if(value == null)  return;
    ...
    public static void odometerChange(View view,int ID){
        final TextView total = view.findViewById(ID);
        String value = BluetoothService.mOWDevice.characteristics.get(MockOnewheelCharacteristicOdometer).value.get();
        if(value == null)  return;
        final String disVal = String.format(Locale.getDefault(), "%.2f",Util.parseF(value));
    ...
    public static void chargeOdometerChange(View view,int ID){
        final TextView total = view.findViewById(ID);
        String value = BluetoothService.mOWDevice.characteristics.get(MockOnewheelCharacteristicOdometerCharge).value.get();
        if(value == null)  return;
        final String disVal = String.format(Locale.getDefault(), "%.2f",Util.parseF(value));
        total.post(new Runnable() {
            @Override
    ...
    public static void rangeOdometerChange(View view,int ID) {
        final TextView total = view.findViewById(ID);
        String value = BluetoothService.mOWDevice.characteristics.get(MockOnewheelCharacteristicOdometerRange).value.get();
        if (value == null) return;
        final String disVal = String.format(Locale.getDefault(), "%.2f", Util.parseF(value));
        total.post(new Runnable() {
            @Override
    ...
    public static void lifeOdometerChange(View view,int ID){
        final TextView total = view.findViewById(ID);
        String value = BluetoothService.mOWDevice.characteristics.get(OnewheelCharacteristicLifetimeOdometer).value.get();
        if(value == null)  return;
    ...
    public static void unitsChange (View view,int ID){
    ...
    public void setCustom(BluetoothUtil bluetoothUtil, int val){
        BluetoothGattCharacteristic lc = bluetoothUtil.getCharacteristic(OWDevice.OnewheelCharacteristicUNKNOWN2);
        if (lc != null) {
