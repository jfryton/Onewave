...
public class WidgetBatteryBar extends AppWidgetProvider {
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager,
        ...
        int progress;
        if (BluetoothService.mService != null && BluetoothService.mService.mOWDevice.characteristics.get(OWDevice.OnewheelCharacteristicBatteryRemaining).value.get() != null){
            progress = Util.parseI(BluetoothService.mService.mOWDevice.characteristics.get(OWDevice.OnewheelCharacteristicBatteryRemaining).value.get());
        }else{
            progress = 0;
        }
        ...
        Drawable progressDrawable = hullo.getProgressDrawable().mutate();
        progressDrawable.setColorFilter(v.getResources().getColor(sources[which]), android.graphics.PorterDuff.Mode.SRC_IN);
        hullo.setProgressDrawable(progressDrawable);*/
